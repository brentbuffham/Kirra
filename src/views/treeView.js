...
import { createDelaunayMeshFromPointCloud, createMeshFromVertices } from "../entities/triangulations/createDelaunayMesh.js";
import { extractEdgesFromPolylines, createConstrainedDelaunayMesh } from "../entities/triangulations/createConstrainedMesh.js";
import { createBVHMeshFromPointCloud } from "../entities/triangulations/createBVHMesh.js";
...
function showContextMenu(event, object, scene, camera) {
    ...
    // Add "Triangulate" option to the context menu
    const triangulateOption = document.createElement("div");
    triangulateOption.textContent = "Tri Delaunay";
    triangulateOption.style.cursor = "pointer";
    ...
    // Add "Triangulate" option to the context menu
    const triangulateConstrained = document.createElement("div");
    triangulateConstrained.textContent = "Tri Constrained";
    triangulateConstrained.style.cursor = "pointer";

    triangulateConstrained.addEventListener("click", () => {
        if (contextMenu && !isRemovingContextMenu) {
            isRemovingContextMenu = true;

            const selectedArray = Array.from(selectedObjects);

            // Extract geometry data (points, polylines, edges)
            const { points, polylines, edges } = extractGeometryData(selectedArray);

            if (points.length > 2) {
                // Perform constrained Delaunay triangulation
                const triangulatedMesh = createConstrainedDelaunayMesh(points, edges, 0x77aaff);
                if (triangulatedMesh) {
                    triangulatedMesh.name = `${selectedArray[0]?.name || "Object"}-MESH`;
                    triangulatedMesh.userData = { uuid: THREE.MathUtils.generateUUID() };
                    triangulatedMesh.userData.isGeneratedMesh = true;
    ...
    const triangulateBVH = document.createElement("div");
    triangulateBVH.textContent = "Tri BVH Mesh";
    triangulateBVH.style.cursor = "pointer";

    triangulateBVH.addEventListener("click", () => {
        // THIS IS FOR THE BVH MESH GENERATION
        if (contextMenu && !isRemovingContextMenu) {
            isRemovingContextMenu = true;
            // Convert selectedObjects Set to an array
            const selectedArray = Array.from(selectedObjects);
            ...
            // Validate if we have enough points
            if (points.length > 2) {
                // Perform BVH mesh generation
                const bvhMesh = createBVHMeshFromPointCloud(points);
                if (bvhMesh) {
                    bvhMesh.name = `${object.name || object.type}-BVH`;
                    bvhMesh.userData = { uuid: THREE.MathUtils.generateUUID() };
                    bvhMesh.userData.isGeneratedMesh = true;
                    scene.add(bvhMesh);
                    console.log("BVH Mesh added to scene.");
                    populatePanelWithSceneObjects(scene, camera);
                } else {
                    console.warn("No BVH mesh generated.");
                }
            } else {
                console.warn("Not enough valid points found for BVH mesh generation.");
            }
            contextMenu.remove();
            contextMenu = null;
            isRemovingContextMenu = false;
