 ...
 *
 * Runs the heightfield grid computation off the main thread.
 * The main thread pre-extracts hole data + charging parameters (since
 * window.loadedCharging is not available in workers), then this worker
 * does the heavy grid computation and triangle generation.
 *
 * Message protocol:
 *   Main → Worker:
...
/**
 * Generate the flyrock shroud surface from pre-computed hole data.
 */
