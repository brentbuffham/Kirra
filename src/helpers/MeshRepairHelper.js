...
// ────────────────────────────────────────────────────────
// T-junction resolution — split edges at mid-edge vertices
// ────────────────────────────────────────────────────────

/**
 * Split a triangle that has Steiner points inside it, using the same
 * local-frame + Delaunator + barycentric-filter approach as
 * SurfaceBooleanHelper.retriangulateWithSteinerPoints().
 *
 * @param {Object} tri - {v0, v1, v2}
...
function splitTriangleWithSteinerPoints(tri, steinerPoints) {
	...
	// ── Build local 2D coordinate frame on triangle plane ──
	...
	// Build points array: 0,1,2 = original vertices, 3+ = Steiner
	var pts = [
	...
	// Project all to local 2D, run Delaunator
	var n = pts.length;
	...
	// Filter sub-triangles by barycentric centroid + area check
	var result = [];
 ...
 * another triangle but that edge hasn't been split. This creates false
 * boundary edges that break weld/stitch/cap repair.
 *
 * Algorithm:
 *   1. Build spatial grid of all unique vertices
 *   2. For each triangle edge, query grid for nearby vertices
 *   3. If a vertex lies on the edge (within tolerance), record as Steiner point
 *   4. Re-triangulate affected triangles using Delaunator + barycentric filter
 *   5. Iterate up to maxPasses to handle cascading T-junctions
 *
...
export function resolveTJunctions(soup, tolerance, maxPasses) {
	...
	var totalSplitTris = 0;
	var totalSteiner = 0;

	// Compute a reasonable cell size from the mesh — use average edge length
	// so each edge only spans ~1-3 cells per dimension (avoids massive grid iteration)
	var edgeLenSum = 0;
	var edgeCount = 0;
	...
	for (var pass = 0; pass < maxPasses; pass++) {
		// ── Step 1: Build spatial grid of all unique vertices ──
		// Cell size ~ average edge length so edge queries touch only ~3x3x3 cells
		...
		// ── Step 2: For each triangle, check edges for T-junction vertices ──
		// Query only the 3x3x3 neighborhood around each edge endpoint (not full bbox)
		var triSteiner = []; // per-triangle array of Steiner points
		var splitCount = 0;
		var steinerCount = 0;
		var tolSq = tolerance * tolerance;
		...
		for (var ti = 0; ti < soup.length; ti++) {
			...
			var tvKeys = [vKey(tv[0]), vKey(tv[1]), vKey(tv[2])];
			var steiners = null;
			...
			for (var ei = 0; ei < 3; ei++) {
				...
				// Edge vector and squared length
				var abx = eB.x - eA.x;
				var aby = eB.y - eA.y;
				var abz = eB.z - eA.z;
				...
				// Endpoint exclusion: t must be in (eps, 1-eps)
				var eps = tolerance / abLen;
				if (eps >= 0.5) continue; // edge too short relative to tolerance

				// Walk along the edge at cell-size intervals, querying the 3x3x3
				// neighborhood at each sample. This keeps iteration O(edgeLen/cellSize)
				// instead of O((edgeLen/cellSize)^3).
				var visited = {};
						...
						for (var dy = -1; dy <= 1; dy++) {
							for (var dz = -1; dz <= 1; dz++) {
								...
								for (var ci = 0; ci < cell.length; ci++) {
									...
									// Skip if V is a vertex of this triangle
									var vk = vKey(V);
									if (vk === tvKeys[0] || vk === tvKeys[1] || vk === tvKeys[2]) continue;

									// Project V onto edge A→B: t = dot(V-A, B-A) / |B-A|²
									var apx = V.x - eA.x;
									var apy = V.y - eA.y;
									var apz = V.z - eA.z;
									var tp = (apx * abx + apy * aby + apz * abz) / abLenSq;

									// Must be interior to edge (not at endpoints)
			...
			if (steiners) {
				// Deduplicate Steiner points for this triangle
				var seen2 = {};
				var unique = [];
				for (var su = 0; su < steiners.length; su++) {
					var sk = vKey(steiners[su]);
					if (!seen2[sk]) {
						seen2[sk] = true;
				...
				splitCount++;
				steinerCount += unique.length;
			}
		}

		if (splitCount === 0) {
			if (pass === 0) {
				console.log("MeshRepairHelper: resolveTJunctions — no T-junctions found");
			} else {
				console.log("MeshRepairHelper: resolveTJunctions — pass " + (pass + 1) + ": no more T-junctions");
			}
			break;
		}

		// ── Step 3: Re-triangulate affected triangles ──
		var newSoup = [];
		...
		console.log("MeshRepairHelper: resolveTJunctions — pass " + (pass + 1) +
			": split " + splitCount + " triangles at " + steinerCount +
			" T-junction vertices (" + soup.length + " → " + newSoup.length + " tris)");

		totalSplitTris += splitCount;
		totalSteiner += steinerCount;
		soup = newSoup;
	}

	if (totalSplitTris > 0) {
		console.log("MeshRepairHelper: resolveTJunctions — total: split " + totalSplitTris +
			" triangles at " + totalSteiner + " T-junction vertices");
	}
